...
class SlidePolygon01Drawable: MyDrawable() {
    ...
    // 最初の描画角度
    private var angleInit = 360f/nVertexMax.toFloat()
    // 現在の描画角度
    ...
    private fun addPoint() {
        ...
        when ( nVertexDv ) {
            // 頂点までたどり着く
            nVertexDvMax-1 -> {
                x = c0.x+r*1f/nVertexDvMax.toFloat()*cos(angle*PI/180f).toFloat()
                y = c0.y+r*1f/nVertexDvMax.toFloat()*sin(angle*PI/180f).toFloat()
                nVertex++
        ...
        if ( ( nVertexDv == 0 ) and ( nVertex == 0 ) ) {
            angle = 360f+angle-angleVertex/2f
            nCnt++
        }

        /*
        Log.d(javaClass.simpleName,"======================================")
        Log.d(javaClass.simpleName,"nCnt[$nCnt]nCntMax[$nCntMax]nVertex[$nVertex]nVertexMax[$nVertexMax]")
        Log.d(javaClass.simpleName,"nVertexDv[$nVertexDv]nVertexDvMax[$nVertexDvMax]angle[$angle]")
